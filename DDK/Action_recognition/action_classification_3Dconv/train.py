...
from constants import BATCH_SIZE, CHECKPOINTS_SOFTMAX_DIR,  loss_accuracy_DIR,  learning_rate, resume_softmax, epochs, device
from constants import warm_up_epochs
from conv_models import Model
...
def cal_flops(model):
    ...
    a = 0
def softmax_train(dsm: Model, train_loader, epoch, optimizer):
    dsm.train()
    correct = 0
    ...
    if epoch%10==0:
        ...
        return None, None
def softmax_test(dsm: Model, test_loader, epoch, early_stopping, early_stopping_flag = False):
...
def start_training(kx_train, ky_train, kx_test, ky_test,  random_seed=1,  trial=1):
    ...
    model = Model(num_channels=1, include_softmax=True, num_class=num_class)
    dsm = model.to(device)
    cal_flops(dsm)
    #optimizer = optim.Adam(dsm.parameters(), lr=learning_rate, weight_decay = 0.001)
    optimizer = optim.SGD(dsm.parameters(), lr=learning_rate, momentum=0.5, weight_decay=0.001)
    ...
    #np.savetxt(os.path.join(loss_accuracy_DIR, 'train_acc_mfcc_nfft1024_'+str(acc_best)+'.txt'), train_accuracy, fmt='%.6f')
    #np.savetxt(os.path.join(loss_accuracy_DIR, 'train_loss_mfcc_nfft1024_'+ str(acc_best)+'.txt'), train_losses, fmt='%.6f')
    ...
    np.savetxt(os.path.join(loss_accuracy_DIR, str(trial), 'test_f1_score_' + str(acc_best) + '.txt'),test_f1_score, fmt='%.6f')
