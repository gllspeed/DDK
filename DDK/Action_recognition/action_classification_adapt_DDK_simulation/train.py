...
def softmax_train(dsm: DeepSpeakerModel, train_loader, epoch, optimizer, noise_mean = None, noise_std = None, map_min = None, map_max = None):
...
def softmax_test(dsm: DeepSpeakerModel, test_loader, epoch, early_stopping, num_class, early_stopping_flag = False, noise_mean = 0.01, noise_std = 0.01, map_min = 40, map_max = 300):
    test_num = BATCH_SIZE_TEST
    dsm.eval()

    ############################################
    #w_add_noise(dsm.fuse_fc1_and_bn1, noise_mean, noise_std, map_min, map_max, clip_min=-0.05, clip_max=0.05)
    #w_add_noise(dsm.fuse_fc2_and_bn2, noise_mean, noise_std, map_min, map_max, clip_min=-1, clip_max=1)
    #w_add_noise(dsm.fuse_fc3_and_bn3, noise_mean, noise_std, map_min, map_max, clip_min=-1, clip_max=1)
    #w_add_noise(dsm.fc4, noise_mean, noise_std, map_min, map_max, clip_min=-1, clip_max=1)
    test_loss = 0
    correct = 0######统计帧级别的准确率
    correct_sample = 0
    targets = []
...
def start_training(kx_train, ky_train, kx_test, ky_test,  random_seed=1,  trial=1, is_DDK = True):
    ...
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=BATCH_SIZE_TEST, shuffle=False, num_workers=1,
                                              pin_memory=True)
    model = DeepSpeakerModel(num_channels=1, include_softmax=True, num_speakers_softmax=num_speakers_softmax, is_MFL=is_DDK)
    dsm = model.to(device)

    ##########加载预训练模型######################
    ###########alpha,beta多维，fc,bn,sigmoid##################
    #pretrained_state = torch.load(
    #    '/home/gaolili/deepLearning_project/action_classification_adapt_DDK/checkpoints/softmax_0.9056603773584906.pt')


    ################alpha,beta多维的模型####################
    #pretrained_state = torch.load(
    #        '/home/gaolili/deepLearning_project/action_classification_adapt_DDK/checkpoints/DDK_3layers/checkpoints-softmax_DDK/softmax_37_0.9018867924528302.pt')

    ##########alpha,beta 一维的模型，alpha,beta固定写到器件，从头开始训练fc权重， 已经训练好权重，用来含噪测试############
    #pretrained_state = torch.load(
    #    '/home/gaolili/deepLearning_project/action_classification_adapt_MFL_simulation/checkpoints/DDK/checkpoints-softmax_DDK/softmax_1_0.8943396226415095.pt')

    #########alpha,beta 一维的模型， alpha,beta训练, 加载这个模型，开始精调
    pretrained_state = torch.load(
        '/home/gaolili/deepLearning_project/action_classification_adapt_DDK/checkpoints/DDK_3layers_a_b/checkpoints-softmax_DDK/softmax_47_0.909433962264151.pt')

    '''
    model_dict = dsm.state_dict()
    pretrained_state = {k: v for k, v in pretrained_state.items() if
                        (k in ['beta', 'alpha'] and v.shape == model_dict[k].shape)}
    ...
    for m in dsm.modules():
        ...
        if isinstance(m, nn.Linear):
            ...
            n, bins, batches = plt.hist(weight.ravel(), 128)
            plt.savefig(
                '/home/gaolili/deepLearning_project/action_classification_adapt_MFL_simulation/checkpoints/DDK/' + str(
                    count) + '_fuse.jpg')
    '''

    early_stopping_flag = False
    early_stopping_flag, accuracy, precision, recall, F1_score, loss_test = softmax_test(dsm, test_loader, 1,
                                                                                         early_stopping,
    ...
    for epoch in range(initial_epoch, epochs):
        scheduler.step()
        acc_train, loss_train = softmax_train(dsm, train_loader, epoch, optimizer, noise_mean = 0.20329, noise_std = 1.14726, map_min = 40, map_max = 300)
        if is_DDK:
        ...
        if epoch % 10 == 0:
            ...
            early_stopping_flag, accuracy, precision, recall, F1_score, loss_test = softmax_test(dsm, test_loader, epoch, early_stopping, num_speakers_softmax, early_stopping_flag, noise_mean = 0.20329, noise_std = 1.14726, map_min = 40, map_max = 300)
