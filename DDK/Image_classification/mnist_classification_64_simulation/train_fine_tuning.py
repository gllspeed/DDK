from mode_fine_tuning import *
import mode_fine_tuning
import torch
...
if __name__ == "__main__":
    for trial in range(1,51):
        ...
        model = mode_fine_tuning.Net().to(device)
        ...
        model.load_state_dict(torch.load(
            '/home/gaolili/deepLearning_project/mnist_classification_64/ckpt_DDK/fc_bn_relu/mnist_7_0.9767.pt'))
        ############alpha, beta的值添加噪声###########
                    ...
                    if acc_best_epoch>=0.9:
                        if input_data_DDK:
                            torch.save(model.state_dict(),
                                       '/home/gaolili/deepLearning_project/mnist_classification_64_simulation/ckpt_DDK_fc_bn_relu_fine_tuning_epoch10/mnist_' + str(trial)+'_'+ str(
                                           acc_best_epoch) + '.pt')
                        else:
                            torch.save(model.state_dict(),
        ...
        np.savetxt(os.path.join('/home/gaolili/deepLearning_project/mnist_classification_64_simulation/result/DDK_fc_bn_relu_fine_tuning_epoch10' ,str(trial), 'accuracy.txt'), test_accuracy, fmt='%.6f')
        np.savetxt(os.path.join('/home/gaolili/deepLearning_project/mnist_classification_64_simulation/result/DDK_fc_bn_relu_fine_tuning_epoch10',str(trial), 'alpha.txt'), alphas, fmt='%.6f')
        np.savetxt(os.path.join('/home/gaolili/deepLearning_project/mnist_classification_64_simulation/result/DDK_fc_bn_relu_fine_tuning_epoch10',str(trial), 'beta.txt'), betas, fmt='%.6f')
