...
from utils import *
from constants import BATCH_SIZE_TRAIN,BATCH_SIZE_TEST,  CHECKPOINTS_SOFTMAX_DIR,  loss_accuracy_DIR,  learning_rate, resume_softmax, epochs, device
...
def softmax_train(dsm: DeepSpeakerModel, train_loader, epoch, optimizer, noise_mean = 0.01, noise_std = 0.01, map_min = 40, map_max = 300):
    ...
    if epoch%10==0:
        ...
        return None, None
'''
def softmax_test(dsm: DeepSpeakerModel, test_loader, epoch, early_stopping, early_stopping_flag = False):
...
def softmax_test(dsm: DeepSpeakerModel, test_loader, epoch, early_stopping, early_stopping_flag = False, noise_mean = 0.01, noise_std = 0.01, map_min = 40, map_max = 300):
    ...
    for i in range(int(len(targets) / num_frames)):
        ...
        for j in range(num_frames):
            if temp_pred[j] == temp_target[j]:
                ...
                correct_temp += 1
        if  correct_temp / num_frames > 0.7:
            correct_sample+=1
    ...
    print('\nTest set: Accuracy: {:.3f}%\n'.format(
        100. * correct_sample / (len(targets) / num_frames)
    ))
    ...
    accuracy_avg = correct_sample / (len(targets) / num_frames)
    accuracy.append(accuracy_avg)
...
def start_training(kx_train, ky_train, kx_test, ky_test,  random_seed=1,  trial=1, is_DDK = True):
    ...
    pretrained_state = torch.load(
        '/home/gaolili/deepLearning_project/3Dvoxel_classification_adapt_DDK/checkpoints/DDK/model_accuracy/1/model/softmax_7_0.88.pt')
    ...
    for epoch in range(initial_epoch, epochs):
        scheduler.step()
        acc_train, loss_train = softmax_train(dsm, train_loader, epoch, optimizer, noise_mean=0.20329,
                                                                                         noise_std=1.14726,
                                                                                         map_min=40,
                                                                                         map_max=300)
        if is_DDK:
        ...
        if epoch % 10 == 0:
            ...
            early_stopping_flag, accuracy, precision, recall, F1_score, loss_test = softmax_test(dsm, test_loader, epoch, early_stopping, early_stopping_flag)
