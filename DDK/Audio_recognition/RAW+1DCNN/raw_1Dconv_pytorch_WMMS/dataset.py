...
class load_voice_print:
    def __init__(self, dataloader_path, training, sed):
            ...
            inputs = data['whaleInputRaw']
            labels = data['whaleTargetRaw']

            #data = scio.loadmat(os.path.join(dataloader_path, 'speaker_fft_nfft512.mat'))
            #target = scio.loadmat(os.path.join(dataloader_path, 'targets_fft_nfft512.mat'))
            #inputs = data['samples']
